...
bool User::exceptionUser()
{
	...
	{
		sendToClient(_curUser, _fd, "462", _parsedCommand[0] + " :Unauthorized command (already registered)", SERVER);
		return true;
	}
	...
	{
		sendToClient(_curUser, _fd, "", _parsedCommand[0] + " :Wrong realname arg!", SERVER);
		return true;
	}
	return false;
}
...
void User::execute()
{
	...
	sendToClient(_curUser, _fd, "001", "Welcome to the Internet Relay Network " + postfix, SERVER);
	sendToClient(_curUser, _fd, "002", "Your host is " + _curUser.getServerName() + ", running version 0.0.1", SERVER);
	sendToClient(_curUser, _fd, "003", "This server was created 24/02/02", SERVER);
	sendToClient(_curUser, _fd, "004", _curUser.getServerName() + " 0.0.1 ", SERVER);
}
